...
{
    auto &&[p, pp, offset, _] = TranslatedCyclerDecider{}.find(known::boydJohnson(), 10'000'000);
    assertEqual(p, 17620);
...
{
    auto &&[p, pp, offset, _] = TranslatedCyclerDecider{}.find(known::bbb4Champion(), 50'000'000);
    assertEqual(p, 1);
...
{
    auto &&[p, pp, offset, _] = TranslatedCyclerDecider{}.find({"1RB0LA_0RC1LA_1RD0RD_1LB1RB"}, 70000000);
...
{
    auto t1 = now();
    auto &&[p, pp, offset, _] = TranslatedCyclerDecider{}.find({"1RB0LC_1RD1LC_0LA1LB_1LC0RD"}, 500000000);
    println(now() - t1);
...
void testCycler()
{
    auto &&[p, pp, offset, _] = CyclerDecider{}.find({"1RB---_1RC1RC_1LC1LB"}, 300);
    assertEqual(p, 2);
    assertEqual(pp, 3);
...
void testCycler2()
{
    auto &&[p, pp, offset, _] = CyclerDecider{}.find({"1RB0RB_1LC0RD_1LA1LB_0LC1RD"}, 2000);
    assertEqual(p, 120);
    assertEqual(pp, 6);
    assertEqual(offset, 0);
    pass("testCycler2");
